...
import {
  I_InitiativesModel, I_MembersModel, I_ProjectModel, I_SolutionsModel
} from './interfaces';
...
export class Database {
  ...
  static file = (dbName: string): string => join(__dirname, `../../../data/${dbName}.json`)

  async initializeModels() {
    const IntitiativeAdapter = new JSONFileSync<Initiative_Data>(Database.file(`initiative`))
    const ProjectsAdapter = new JSONFileSync<Projects_Data>(Database.file(`projects`));
    const SolutionsAdapter = new JSONFileSync<Solutions_Data>(Database.file(`solutions`));
    const MembersAdapter = new JSONFileSync<Members_Data>(Database.file(`members`));
    this.initiative = new LowWithLodash<Initiative_Data>(IntitiativeAdapter, []);
