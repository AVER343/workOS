...
import { I_ProjectModel, I_SolutionTableModel, I_SolutionsModel } from "../../utils/db/interfaces";
...
export const solutionTableSlice = createSlice({
    ...
    reducers: {
        editSolutionTable(state, action: { payload: I_SolutionTableModel }) {
            ...
            let index = solutionTables.findIndex(table => table.table_id == action.payload.table_id);
        ...
        getSolutionTable(state, action: { payload: { solution_id: string; } }) {
            console.log({ action });
            const solution = DB_INSTANCE.solutions.adapter.read().find(solution => solution.id == action.payload.solution_id);
            const solutionTable = DB_INSTANCE.solutionTable.adapter.read().find(table => table.table_id == solution.table_id[0])
